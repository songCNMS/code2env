...
- **side-effect 风险不能只看 callee basename**：`FunctionCandidate.calls` 为兼容仍保留 basename，但 `possible_side_effect` 判定应检查 AST call target，否则 `payload.get()`/`cache.get()` 会被误判成 HTTP `get`；真实 qlib scan 从 old get-only=93 降到 patched get-only=6。(task043, PR#29)
